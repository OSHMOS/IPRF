...
# from nnfm_loss import NNFMLoss, match_colors_for_image_set, nn_feat_replace, cos_loss
from our_loss import NNFMLoss, match_colors_for_image_set, nn_feat_replace, cos_loss
...
from iid_extractor.Network import DecScaleClampedIllumEdgeGuidedNetworkBatchNorm
from iid_extractor.Utils import mor_utils
...
# PIE-NET pretrained 불러오기
modelSaveLoc = './iid_extractor/ckpt/real_world_model.t7'
...
#### PRF parameters
parser.add_argument("--shading_weight", type=float, default=5e-2, help="shading loss weight")
#### END of PRF parameters
...
with torch.cuda.device(1):
    ...
    if style.shape[2:][0] > style.shape[2:][-1]:
        style = F.interpolate(style, size=(int(content_long_side / 2), int(content_short_side / 2)), mode='bilinear')
    # h < w 면 1, 3, content short side, content long side
    else:
        style = F.interpolate(style, size=(int(content_short_side / 2), int(content_long_side / 2)), mode='bilinear')

    if style.shape[1] == 4:
        style = style[:, :3, :, :]  # alpha channel remove 

    #IID
    IID_style = net(style) # style image to PIE-Net
    # albedo (equal reflectance)
    albedo_style = IID_style['reflectance'] # style image albedo
    # shading
    shading_style = IID_style['shading'] # style image shading

seed_everything(20200823)

albedo_style = albedo_style.to('cuda:0')
albedo_style_img = albedo_style.squeeze(0).permute(1, 2, 0).contiguous()

shading_style = shading_style.to('cuda:0')
shading_style_img = shading_style.squeeze(0).permute(1, 2, 0).contiguous()

global_start_time = datetime.now()

if not args.no_pre_ct:
    # dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, albedo_style_img) # h, w, 3
    dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, style_img) # h, w, 3
...
while True:
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def postProcessing(img):
                    ...
                    img = (img / img.max()) * 255  # scaling & multiple 255
                    normalize_img = img.astype(np.uint8)

                    # BGR to RGB (only 3 channel image)
                    if normalize_img.shape[-1] == 3:
                        result = cv2.cvtColor(normalize_img, cv2.COLOR_BGR2RGB)
                    else:
                        result = normalize_img

                    return result
                # def compute_image_loss():
                def compute_image_loss(device):
                    with torch.no_grad():
                        cam = svox2.Camera(
                    ...
                    # rendering images
                    visuals = args.train_dir + '/albedo_results'
                    result_epoch = os.path.join(visuals, str(epoch_id))
                    ...
                    with torch.cuda.device(1):
                        ...
                        # albedo (equal reflectance)
                        albedo_content = IID_content['reflectance'] # content image albedo
                        albedo_styled = IID_styled['reflectance'] # stylized albedo

                        # shading
                        shading_content = IID_content['shading'] # content image shading                       
                        shading_styled = IID_styled['shading'] # stylized image shading
                    ...
                    albedo_content = albedo_content.to('cuda:0')
                    albedo_styled = albedo_styled.to('cuda:0')
                    ...
                    cv2.imwrite(f'{result_epoch}/albedo_style.png', postProcessing(albedo_style.squeeze()))

                    shading_style = shading_style_img.permute(2, 0, 1).unsqueeze(0).contiguous() # 1,3,h,w
                    cv2.imwrite(f'{result_epoch}/shading_style.png', postProcessing(shading_style.squeeze()))

                    # style
                    albedo_style_transfer_loss = nnfm_loss_fn(
                        F.interpolate(
                            albedo_styled,
                            size=None,
                            scale_factor=1.0,
                    ...
                    # contents
                    shading_content_loss = nnfm_loss_fn(
                        F.interpolate(
                            shading_styled_3ch,
                            size=None,
                            scale_factor=1.0,
                        ...
                        albedo_style_img.permute(2, 0, 1).unsqueeze(0),
                        blocks=[
                            args.vgg_block,
                        ],
                        loss_names=["content_loss"],
                        # loss_names=["shading_loss"], # w/o vgg
                        contents=F.interpolate(
                            shading_content_3ch,
                            size=None,
                            scale_factor=1.0,
                    ...
                    shading_content_loss["img_tv_loss"] = img_tv_loss
                    shading_content_loss["content_loss"] *= args.shading_weight
                    # shading_content_loss["shading_loss"] *= args.shading_weight # w/o vgg
                    
                    loss_dict = {**albedo_style_transfer_loss, **shading_content_loss}
                ...
                rgb_pred_grad, loss_dict = compute_image_loss(device)
                rgb_pred = []
    ...
    if epoch_id >= args.mse_num_epoches + args.nnfm_num_epoches:
        if not args.no_post_ct:
            ...
            # dset.rays.gt, color_tf = match_colors_for_image_set(dset.rays.gt, albedo_style_img)
