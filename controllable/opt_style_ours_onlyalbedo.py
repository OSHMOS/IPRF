...
with torch.cuda.device(1):
    ...
    #IID
    IID_style = net(style) # PIE-NET에 스타일 이미지를 입력한 결과
    # albedo (equal reflectance)
    albedo_style = IID_style['reflectance'] # 스타일 이미지의 albedo
    # shading
    shading_style = IID_style['shading'] # 스타일 이미지의 shading
        ...
        for iter_id, batch_begin in pbar:
            ...
            if optim_type == "ray":
                ...
                def postProcessing(img):
                    ...
                    if img.ndim == 3 and img.shape[0] <= 4:  # 채널이 4개 이하인 경우
                        img = img.transpose((1, 2, 0))  # (C, H, W) -> (H, W, C)

                    # normalize
                    img[img < 0] = 0  # 0보다 작은 값은 0으로 clipping
                    img = (img / img.max()) * 255  # 0~1 사이로 scaling 후 255를 곱함
                    normalize_img = img.astype(np.uint8)

                    # BGR to RGB (3채널 이미지인 경우에만)
                ...
                def compute_image_loss(device):
                    ...
                    # rendering images # 나중에 r, s 이미지 저장용
                    ...
                    with torch.cuda.device(1):
                        ...
                        # albedo (equal reflectance)
                        albedo_styled = IID_styled['reflectance'] # 스타일화된 이미지의 albedo
                    ...
                    # ours 손실 함수 입력 사이즈는 384x512, 스타일은 높이가 길면 512x384임
                    seed_everything(20200823)
                    ...
                    albedo_style_transfer_loss["img_tv_loss"] = img_tv_loss
                    
                    loss_dict = albedo_style_transfer_loss
