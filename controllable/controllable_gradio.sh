...
ckpt_iprf_ctrl=../opt/ctrl/ckpts/${data_type}/${SCENE}_${STYLE}_ctrl
...
if [[ ! -f "${ckpt_iprf_ctrl}/only_shading/ckpt.npz" ]]; then
    CUDA_VISIBLE_DEVICES=0,1 python opt_style_ours_onlyshading.py -t ${ckpt_iprf_ctrl}/only_shading ${data_dir} \
    ...
    CUDA_VISIBLE_DEVICES=0,1 python render_imgs.py ${ckpt_iprf_ctrl}/only_shading/ckpt.npz ${data_dir} \
                        --render_path --no_imsave
fi

if [[ ! -f "${ckpt_iprf_ctrl}/only_albedo/ckpt.npz" ]]; then
    CUDA_VISIBLE_DEVICES=0,1 python opt_style_ours_onlyalbedo.py -t ${ckpt_iprf_ctrl}/only_albedo ${data_dir} \
                    -c configs/llff_fixgeom.json \
                    --init_ckpt ${ckpt_svox2}/ckpt.npz \
                    --style ${style_img} \
                    --mse_num_epoches 2 --nnfm_num_epoches 10 \
    ...
    CUDA_VISIBLE_DEVICES=0,1 python render_imgs.py ${ckpt_iprf_ctrl}/only_albedo/ckpt.npz ${data_dir} \
                        --render_path --no_imsave
fi
